#!/usr/bin/env python3
"""
Fix all network consolidated audit files with correct baseline structure
"""

import re
from pathlib import Path

def extract_baseline_controls(baseline_path):
    """Extract control IDs from baseline file"""
    baseline_ids = set()
    try:
        with open(baseline_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
        for match in re.finditer(r'description\s*:\s*"(\d+\.\d+)', content):
            baseline_ids.add(match.group(1))
    except:
        pass
    return baseline_ids

def get_baseline_header_footer(baseline_path):
    """Extract header and footer from baseline file"""
    with open(baseline_path, 'r', encoding='utf-8', errors='ignore') as f:
        content = f.read()

    # Header: everything before first <custom_item>
    match = re.search(r'^(.*?)(?=<custom_item>)', content, re.DOTALL)
    header = match.group(1) if match else ""

    # Footer: everything after last </custom_item>
    match = re.search(r'(</custom_item>.*)$', content, re.DOTALL)
    footer = match.group(1) if match else "</check_type>"

    return header, footer

def rebuild_consolidated(input_path, baseline_ids, header, footer):
    """Rebuild consolidated file with baseline structure"""
    with open(input_path, 'r', encoding='utf-8', errors='ignore') as f:
        content = f.read()

    controls = []
    pattern = r'(#?)\s*<custom_item>.*?</custom_item>'

    for match in re.finditer(pattern, content, re.DOTALL):
        control_block = match.group(0)
        # Clean control by removing all # prefixes
        clean_control = re.sub(r'^#\s*', '', control_block, flags=re.MULTILINE)

        # Extract ID
        desc_match = re.search(r'description\s*:\s*"(\d+\.\d+)', clean_control)
        control_id = desc_match.group(1) if desc_match else None

        controls.append({'id': control_id, 'text': clean_control})

    # Build output
    output_lines = [header]

    for ctrl in controls:
        if ctrl['id'] and ctrl['id'] in baseline_ids:
            output_lines.append(ctrl['text'])
        else:
            commented = '\n'.join('#' + line if line.strip() else line
                                 for line in ctrl['text'].split('\n'))
            output_lines.append(commented)
        output_lines.append('')

    output_lines.append(footer)
    return '\n'.join(output_lines)

# Map of all network audit types
audits = {
    'ASA': {
        'baseline': Path(r'C:\PySC\TAP\Output\Processed\Normalized\HTH_Baseline_NetASA_26091508.audit'),
        'input': Path(r'C:\PySC\TAP\Output\Processed\For_Gap\ASA.audit'),
        'output': Path(r'C:\PySC\TAP\Output\Consolidated\ASA_all_audits.audit'),
    },
    'F5': {
        'baseline': Path(r'C:\PySC\TAP\Output\Processed\Normalized\HTH_Baseline_NetF5_26091508.audit'),
        'input': Path(r'C:\PySC\TAP\Output\Processed\For_Gap\F5.audit'),
        'output': Path(r'C:\PySC\TAP\Output\Consolidated\F5_all_audits.audit'),
    },
    'IOS': {
        'baseline': Path(r'C:\PySC\TAP\Output\Processed\Normalized\HTH_Baseline_NetIOS_AuditFile_26091508.audit'),
        'input': Path(r'C:\PySC\TAP\Output\Processed\For_Gap\IOS.audit'),
        'output': Path(r'C:\PySC\TAP\Output\Consolidated\IOS_all_audits.audit'),
    },
    'NXOS': {
        'baseline': Path(r'C:\PySC\TAP\Output\Processed\Normalized\HTH_Baseline_NETNXOS_26091508.audit'),
        'input': Path(r'C:\PySC\TAP\Output\Processed\For_Gap\NXOS.audit'),
        'output': Path(r'C:\PySC\TAP\Output\Consolidated\NXOS_all_audits.audit'),
    },
    'PAFW': {
        'baseline': Path(r'C:\PySC\TAP\Output\Processed\Normalized\HTH_Baseline_NETPAFW_26091508.audit'),
        'input': Path(r'C:\PySC\TAP\Output\Processed\For_Gap\PAFW.audit'),
        'output': Path(r'C:\PySC\TAP\Output\Consolidated\PAFW_all_audits.audit'),
    },
    'RHEL': {
        'baseline': Path(r'C:\PySC\TAP\Output\Processed\Normalized\HTH_Baseline_RHEL_26091508.audit'),
        'input': Path(r'C:\PySC\TAP\Output\Processed\For_Gap\RHEL.audit'),
        'output': Path(r'C:\PySC\TAP\Output\Consolidated\RHEL_all_audits.audit'),
    },
}

print("Rebuilding consolidated audit files with correct baseline structure...\n")

for audit_type, paths in audits.items():
    print(f"Processing {audit_type}...")

    if not paths['baseline'].exists():
        print(f"  ❌ Baseline not found")
        continue
    if not paths['input'].exists():
        print(f"  ❌ Input not found")
        continue
    if not paths['output'].exists():
        print(f"  ❌ Output not found")
        continue

    baseline_ids = extract_baseline_controls(paths['baseline'])
    header, footer = get_baseline_header_footer(paths['baseline'])

    print(f"  Baseline IDs: {len(baseline_ids)}")

    # Count before
    with open(paths['output'], 'r', encoding='utf-8', errors='ignore') as f:
        before = f.read()
    before_uncommented = len(re.findall(r'^\s*<custom_item>', before, re.MULTILINE))

    # Rebuild
    fixed = rebuild_consolidated(paths['input'], baseline_ids, header, footer)

    # Count after
    after_uncommented = len(re.findall(r'^\s*<custom_item>', fixed, re.MULTILINE))

    # Write
    with open(paths['output'], 'w', encoding='utf-8') as f:
        f.write(fixed)

    print(f"  Before: {before_uncommented} uncommented")
    print(f"  After:  {after_uncommented} uncommented")
    print(f"  ✅ Fixed")

print("\n" + "="*60)
print("All consolidated files rebuilt!")
print("="*60)
