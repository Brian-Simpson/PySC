...
foreach ($computer in $computers) {
    ...
    try {
        ...
        foreach ($computerName in $computerList) {
            Write-Host "Checking WinRM and Execution Policy on $computerName..."

            try {
                # Test WinRM connectivity
                Test-WSMan -ComputerName $computerName -ErrorAction Stop

                Write-Host "  WinRM is running and accessible on $computerName."

                # Get the effective execution policy
                $executionPolicy = Invoke-Command -ComputerName $computerName -ScriptBlock { Get-ExecutionPolicy }

                Write-Host "  Effective Execution Policy on $computerName : $executionPolicy"
            }
            catch {
                # Handle errors related to Test-WSMan or Invoke-Command
                Write-Host "  Error connecting to $computerName or getting execution policy: $($_.Exception.Message)"
                Write-Host "  WinRM might not be configured, or a firewall might be blocking the connection."
            }
        }
