...
Write-Log "Please enter credentials for remote execution..." -Level "INFO"
Write-Log "These credentials will be used to connect to all target systems." -Level "INFO"
Write-Log "Format: DOMAIN\username or username@domain.com" -Level "INFO"
$credential = Get-Credential -Message "Enter credentials for remote execution on target systems"

if (-not $credential) {
    Write-Log "No credentials provided. Script cannot continue." -Level "ERROR"
    exit
}

Write-Log "Using credentials for user: $($credential.UserName)" -Level "INFO"
...
foreach ($computer in $computers) {
    ...
    try {
        ...
        try {
            ...
            Write-Log "WinRM is running and accessible on $computer" -Level "INFO"
            
            # Get the execution policy
            try {
                $executionPolicy = Invoke-Command -ComputerName $computer -ScriptBlock { Get-ExecutionPolicy } -Credential $credential -ErrorAction Stop
                $result.ExecutionPolicy = $executionPolicy
                Write-Log "Execution Policy on $computer : $executionPolicy" -Level "INFO"
            }
            catch {
                $result.ExecutionPolicy = "Error: $($_.Exception.Message)"
                Write-Log "Error getting execution policy on $computer : $($_.Exception.Message)" -Level "ERROR"
            }
        }
        catch {
...
$results | Format-Table -Property ComputerName, Success, PingStatus, WinRMStatus, ExecutionPolicy, ExecutionTime, ApplicationCount, Error -AutoSize
