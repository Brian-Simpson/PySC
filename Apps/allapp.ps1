        ...
        try {
            ...
            if ($Credential) {
                try {
                    ...
                    Write-Host "✗ Primary WinRM authentication failed: $($_.Exception.Message)" -ForegroundColor Red
                    Write-Host "Error Code: 0x$($_.Exception.HResult.ToString('X8'))" -ForegroundColor Red
                    
                    # Try alternative authentication methods for error 0x80330AB1
                    if ($_.Exception.HResult -eq -2144108367) { # 0x80330AB1
                        Write-Host "Attempting alternative authentication methods..." -ForegroundColor Yellow
                        
                        # Method 1: Try with explicit authentication type
                        try {
                            Write-Host "Trying with Negotiate authentication..." -ForegroundColor Yellow
                            $sessionOption = New-PSSessionOption -NoMachineProfile
                            $testSession = New-PSSession -ComputerName $ComputerName -Credential $Credential -Authentication Negotiate -SessionOption $sessionOption -ErrorAction Stop
                            Remove-PSSession $testSession
                            Write-Host "✓ Negotiate authentication successful" -ForegroundColor Green
                            $useCredentials = $true
                            $authMethod = "Negotiate"
                        }
                        catch {
                            Write-Host "✗ Negotiate authentication failed: $($_.Exception.Message)" -ForegroundColor Red
                            
                            # Method 2: Try with Kerberos
                            try {
                                Write-Host "Trying with Kerberos authentication..." -ForegroundColor Yellow
                                $testSession = New-PSSession -ComputerName $ComputerName -Credential $Credential -Authentication Kerberos -SessionOption $sessionOption -ErrorAction Stop
                                Remove-PSSession $testSession
                                Write-Host "✓ Kerberos authentication successful" -ForegroundColor Green
                                $useCredentials = $true
                                $authMethod = "Kerberos"
                            }
                            catch {
                                Write-Host "✗ Kerberos authentication failed: $($_.Exception.Message)" -ForegroundColor Red
                                
                                # Method 3: Try with Basic authentication (if enabled)
                                try {
                                    Write-Host "Trying with Basic authentication..." -ForegroundColor Yellow
                                    $testSession = New-PSSession -ComputerName $ComputerName -Credential $Credential -Authentication Basic -SessionOption $sessionOption -ErrorAction Stop
                                    Remove-PSSession $testSession
                                    ...
                                    # Method 4: Try with NTLM
                                    try {
                                        Write-Host "Trying with NTLM authentication..." -ForegroundColor Yellow
                                        $testSession = New-PSSession -ComputerName $ComputerName -Credential $Credential -Authentication Ntlm -SessionOption $sessionOption -ErrorAction Stop
                                        Remove-PSSession $testSession
                                        Write-Host "✓ NTLM authentication successful" -ForegroundColor Green
                                        $useCredentials = $true
                                        $authMethod = "Ntlm"
                                    }
                                    catch {
                                        Write-Host "✗ All authentication methods failed. Unable to authenticate to $ComputerName" -ForegroundColor Red
                                        ...
                                        Write-Host "Skipping $ComputerName due to authentication failure" -ForegroundColor Yellow
                                        return $null
                                    }
                                }
                            }
