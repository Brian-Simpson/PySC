...
# $password = ConvertTo-SecureString "l" -AsPlainText -Force # Replace with actual password
# HTHJP86DK3
# HTH9S9CGB3
# HTH4VCR9K3
# HTH992QSV3
...
function Invoke-ComputerInventory {
    ...
    if ($IsLocal) {
        ...
        try {
            ...
            Write-Host "1. On the remote computer, run as Administrator:" -ForegroundColor Cyan
            Write-Host "   Enable-PSRemoting -Force" -ForegroundColor White
            Write-Host "   Set-WSManQuickConfig -Force" -ForegroundColor White
            Write-Host "2. Check Windows Firewall:" -ForegroundColor Cyan
            Write-Host "   Allow 'Windows Remote Management' through firewall" -ForegroundColor White
            Write-Host "3. Verify credentials are correct for domain: $($Credential.UserName)" -ForegroundColor Cyan
            Write-Host "4. Ensure both computers are domain-joined or in trusted relationship" -ForegroundColor Cyan
            
            Write-Host "Skipping $ComputerName due to connection failure" -ForegroundColor Yellow
            return $null
        }
        
        # Test actual PowerShell remoting capability
        try {
            Write-Host "Verifying PowerShell remoting functionality..." -ForegroundColor Yellow
            $testScriptBlock = { 
                return @{
            ...
            if ($useCredentials) {
                $remoteInfo = Invoke-Command -ComputerName $ComputerName -Credential $Credential -ScriptBlock $testScriptBlock -ErrorAction Stop
            } else {
                $remoteInfo = Invoke-Command -ComputerName $ComputerName -ScriptBlock $testScriptBlock -ErrorAction Stop
            }
            
            Write-Host "✓ Remote PowerShell session established successfully" -ForegroundColor Green
            Write-Host "  Remote Computer: $($remoteInfo.ComputerName)" -ForegroundColor Gray
            Write-Host "  Running as: $($remoteInfo.UserContext)" -ForegroundColor Gray
            Write-Host "  PowerShell Version: $($remoteInfo.PowerShellVersion)" -ForegroundColor Gray
            Write-Host "  Execution Policy: $($remoteInfo.ExecutionPolicy)" -ForegroundColor Gray
            Write-Host "  Temp Directory: $($remoteInfo.TempPath)" -ForegroundColor Gray
        }
        catch {
        ...
        try {
            ...
            if ($_.Exception.Message -like "*timeout*") {
                Write-Host "This appears to be a timeout issue. The remote system may be processing a large number of files." -ForegroundColor Yellow
                Write-Host "Consider reducing the search scope or increasing the timeout." -ForegroundColor Yellow
            }
...
# Run as Administrator on each target computer:
