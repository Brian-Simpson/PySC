...
REAL_KEYS = {
    ...
    "expect", "not_expect", "regex",
    # VMware ESXi-specific keys
    "xsl_stmt",
}
...
def _parse_block(lines, i, end_pattern):
    """Parse key:value lines until end_pattern is matched.
    Returns (fields_as_list_of_tuples, i_after_closing_tag).
    fields is a list of (key, value) to preserve repeated keys like xsl_stmt.
    """
    fields = []
    key = None
    buf = []
    ...
    if key is not None:
        fields.append((key, "\n".join(buf).strip()))
    return fields, i + 1  # +1 to skip closing tag
...
def parse_document(lines):
    ...
    while i < len(lines):
        ...
        if re.match(r'\s*<report\s+type:"PASSED">', line):
            i += 1
            fields, i = _parse_block(lines, i, r"\s*</report>")
        ...
        if re.match(r"\s*(<custom_item>|&lt;custom_item&gt;)", line):
            i += 1
            fields, i = _parse_block(lines, i, r"\s*(</custom_item>|&lt;/custom_item&gt;)")
...
def transform_fields(fields, variables, after_passed, desc_counter, unknown_keys):
    """Apply normalization transforms to a list of (key, value) pairs.
    Returns (pairs, new_desc_counter).
    """
    pairs = []
    for k, v in fields:
        ...
        if k == "see_also":
            ...
            # Strip the "(L1)"/"(L2)" level tag that ESXi descriptions include
            clean = re.sub(r'^\(L\d+\)\s*', '', clean)
            new_desc = f"\"1.{desc_counter:03d} - ESXi - {clean}\""
