...
REAL_KEYS = {
    ...
    # Cisco NX-OS-specific keys
    "cmd", "expect", "not_expect", "regex",
    "context", "item", "only_show_cmd_output",
    "content", "required",
    "max_occurrences",
    "min_occurrences",
}
...
def parse_document(lines):
    ...
    while i < len(lines):
        ...
        if m_item:
            ...
            buf = []
            i += 1
            while i < len(lines) and not re.match(r"\s*</item>", lines[i]):
                m = re.match(r"\s*([A-Za-z0-9_]+)\s*:\s*(.*)", lines[i])
                if m and not (key and _in_open_quote(buf)):
                    if key:
                        fields[key] = "\n".join(buf).strip()
            ...
            if key:
                fields[key] = "\n".join(buf).strip()
            document.append({"type": "item", "fields": fields, "indent": indent})
            i += 1  # skip </item>
            continue
...
def transform_fields(fields, variables, after_passed, desc_counter, unknown_keys):
    ...
    for k, v in fields.items():
        ...
        if k == "see_also":
            ...
            new_desc = f"\"1.{desc_counter:04d} - CSNXOS - {clean}\""
...
def emit(document, variables):
    ...
    for node in document:
        if node["type"] not in ("custom_item", "item", "report-warning", "report-passed"):
            continue
    ...
    for node in document:
        if node["type"] == "text":
            ...
            for k, v in rendered_blocks[block_idx]:
                output.append(f"{indent}  {k.ljust(width)} : {v}")
            output.append(f"{indent}</item>")
            block_idx += 1
