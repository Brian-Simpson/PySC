...
try:
    from pysc_block_parser import extract_variables, parse_document  # type: ignore[reportMissingImports]
except ModuleNotFoundError:
    # Fallback for environments where the script directory is not in import roots.
    import importlib.util

    parser_path = os.path.join(SCRIPT_DIR, "pysc_block_parser.py")
    spec = importlib.util.spec_from_file_location("pysc_block_parser", parser_path)
    if spec is None or spec.loader is None:
        raise
    _parser_mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(_parser_mod)
    extract_variables = _parser_mod.extract_variables
    parse_document = _parser_mod.parse_document
...
REAL_KEYS = {
    ...
    "powershell_args", "powershell_script", "key_item",
...
BASELINE_MSSRV_CSV_PATH = r'C:\PySC\actual_audit_inputs\Baseline_MSSRV.csv'
MERGED_MSSRV_CSV_PATH = os.path.join(SCRIPT_DIR, 'Merged_2607.csv')
DESCRIPTION_MATCH_XLSX_PATH = os.path.join(SCRIPT_DIR, 'Baseline_vs_Merged_MSSRV_Description_Matches.xlsx')
...
def _convert_policy_item_to_audit_powershell(fields):
    ...
    if ptype == 'AUDIT_POLICY_SUBCATEGORY':
        ...
        converted['powershell_args'] = f'"{ps_script}"'
        converted['check_type'] = 'CHECK_REGEX'
        return converted
...
def _convert_remaining_item_to_audit_powershell(fields):
    ...
    converted['powershell_args'] = f'"{script}"'
    converted['check_type'] = original_check_type or 'CHECK_REGEX'
    return converted
...
def _finalize_audit_powershell_fields_for_emit(fields):
    if _norm_upper_token(fields.get('type', '')) != 'AUDIT_POWERSHELL':
    ...
    ps_args = fields.get('powershell_args', fields.get('powershell_script', ''))
    ...
    if ps_args:
        ...
        finalized['powershell_args'] = ps_args

        # Preserve comparison semantics (for example CHECK_REGEX) so
        # value_data patterns are evaluated correctly by the scanner.
        check_type = str(fields.get('check_type', '') or '').strip()
        if 'PERFORM VOLUME MAINTENANCE TASKS IS SET TO ADMINISTRATORS' in desc_upper:
            check_type = 'CHECK_REGEX'
        if check_type:
            finalized['check_type'] = check_type
        else:
            expected_text = _strip_outer_quotes_once(finalized.get('value_data', ''))
            if re.search(r'(\^|\$|\[|\]|\(|\)|\{|\}|\|)|\\d|\\s|\.\*|\(\?:', expected_text):
                finalized['check_type'] = 'CHECK_REGEX'
        ...
        if 'ACCOUNT - LOCAL ADMIN ACCOUNT' in desc_upper:
            ...
            finalized['powershell_args'] = f'"{_build_sid_account_status_powershell_script("500")}"'
            finalized['check_type'] = 'CHECK_EQUAL'
...
CONTROL_KEY_FIELDS_BY_TYPE = {
    ...
    'AUDIT_POWERSHELL': ['powershell_args'],
...
def _classify_account_powershell_rule(fields):
    script = _norm_upper_token(_strip_outer_quotes_once(fields.get('powershell_args', fields.get('powershell_script', ''))))
    desc = _norm_upper_token(fields.get('description', ''))
...
def derive_evaluated_item_key(fields):
    ...
    if ctype == 'AUDIT_POWERSHELL':
        ps_raw = _norm_token(fields.get('powershell_args', ''))
...
def derive_control_key_parts(fields):
    ...
    if control_type == 'AUDIT_POWERSHELL':
        parts = []
        ps_val = summarize_powershell_audit_target(fields.get('powershell_args', ''))
        if ps_val:
            parts.append(('powershell_args', ps_val))

        check_type_val = _strip_outer_quotes_once(fields.get('check_type', ''))
        if check_type_val and not ps_val.lower().startswith('auditpol '):
            parts.append(('check_type', check_type_val))
...
WINDOWS_PARSE_POLICY = {'allow_repair_fallback': False, 'repair_pipeline': ()}
...
def _render_custom_item_block(fields, platform_hint, ordinal, apply_prefix=True):
    ...
    merged_fields = _finalize_audit_powershell_fields_for_emit(merged_fields)
    ...
    ps_script = _strip_outer_quotes_once(merged_fields.get('powershell_args', merged_fields.get('powershell_script', '')))
...
def _target_check_block(platform):
    ...
    fields = OrderedDict()
    fields['type'] = 'AUDIT_POWERSHELL'
    fields['description'] = description
    fields['info'] = '"Target OS validation for baseline applicability."'
    fields['solution'] = '"Validate that the target operating system matches this baseline before applying subsequent controls."'
    fields['reference'] = '""'
    fields['see_also'] = f'"{SEE_ALSO_REPLACEMENT}"'
    fields['value_type'] = 'POLICY_TEXT'
    fields['value_data'] = value_data
    fields['powershell_args'] = '"$R = Get-ItemProperty -Path \'Registry::HKLM\\Software\\Microsoft\\Windows Nt\\Currentversion\' -ErrorAction SilentlyContinue; $V = $R.PSObject.Properties[\'ProductName\'].Value; Write-Output $V;"'
    fields['check_type'] = 'CHECK_REGEX'
    return _render_custom_item_block(fields, platform, -1, apply_prefix=False)
...
def _is_target_os_applicability_control(fields):
    ...
    powershell = _norm_upper_token(_strip_outer_quotes_once(fields.get('powershell_args', '')))
...
def _is_rapid7_insight_agent_control(fields):
    desc = _norm_upper_token(_strip_outer_quotes_once(fields.get('description', '')))
    return 'SERVICES - RAPID7 INSIGHT AGENT SERVICE' in desc
...
def _append_condition_block(out_lines, *, platform, active_rows, inactive_rows, warning_description, ordinal_start, dc_only=False, include_target_gate=True):
    if include_target_gate:
        ...
        if dc_only:
            ...
            fixed_target_desc = '"Windows Server is installed"' if platform == 'MSSRV' else '"Windows Workstation is installed"'
            for i in range(len(out_lines)):
                if re.match(r'^\s*description\s*:', out_lines[i]):
                    out_lines[i] = f'  description     : {fixed_target_desc}'
                    break
...
def _dedupe_merged_rows(merged_rows):
    ...
    for row in merged_rows:
        ...
        # Prefer active rows when a duplicate exists in both active/inactive forms.
        if (not existing.get('active')) and row.get('active'):
...
def _repair_malformed_quoted_field_lines(text, target_keys=None):
    changed = False
    out_lines = []
    keys = {k.lower() for k in (target_keys or {'solution', 'sql_expect', 'xsl_stmt'})}
    ...
    for platform in ('MSWRK', 'MSSRV'):
        ...
        for key in all_keys:
            ...
            if _is_domain_controller_probe_control(fields):
                continue
            force_commented = platform == 'MSSRV' and _is_rapid7_insight_agent_control(fields)
            desc = _strip_outer_quotes_once(fields.get('description', ''))
            merged_rows.append({
                'key': key,
                'active': (key in master_items) and (not force_commented),
                'fields': fields,
...
def emit(document, variables, platform_hint=''):
    ...
    for node in document:
        ...
        custom_fields = _finalize_audit_powershell_fields_for_emit(custom_fields)
...
def parse_cli_args():
    ...
    return input_arg, out_arg, catalog_flag, export_duplicates, open_vscode, install_auditlang, strict_mode


def main():
    input_arg, out_arg, catalog_flag, export_duplicates, open_vscode, install_auditlang, strict_mode = parse_cli_args()
